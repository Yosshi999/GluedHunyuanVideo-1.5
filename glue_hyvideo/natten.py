...
class NattenTiledFlexAttnProcessor:
    def __init__(
        self,
        canvas_dims: Tuple[int, int, int],
        ...
        if self.block_mask is None:
            ...
            def mask_mod(b, h, q_idx, kv_idx):
                ...
                return torch.where(
                    ...
                    (q_idx < image_len_withpad) & (kv_idx < image_len_withpad),
                    (q0 - k0 >= -self.kernel_dims[0]//2 * tile_dims[0]) & (q0 - k0 < (self.kernel_dims[0]+1)//2 * tile_dims[0]) &
                    (q1 - k1 >= -self.kernel_dims[1]//2 * tile_dims[1]) & (q1 - k1 < (self.kernel_dims[1]+1)//2 * tile_dims[1]) &
                    (q2 - k2 >= -self.kernel_dims[2]//2 * tile_dims[2]) & (q2 - k2 < (self.kernel_dims[2]+1)//2 * tile_dims[2]),
                    True,
                )
